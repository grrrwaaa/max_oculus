...
#include "OVR_CAPI.h"
#include "OVR_Types.h"  // for static_assert() needed in OVR_CAPI_GL.h
...
public:
    ...
    int			fullview;
    int         sdk;        // whether to use SDK rendering
    int			multisample;    // for sdk rendering
    double      predict;    // time (in ms) to predict ahead tracking
	
	t_symbol *	texture;	// the name of the input texture to display on the rift
    ...
    void configure_sdk() {
		...
		#ifdef  WIN_VERSION
		if (0 == (ovrHmdCap_ExtendDesktop & ovrHmd_GetEnabledCaps(hmd))) {
			object_warn(&ob, "direct mode rendering is not supported in a plugin; please switch the Oculus driver to extended mode");
		}
		#endif
		
		for (i=0; i<2; i++) {
			eyeTextures[i].OGL.Header.API = ovrRenderAPI_OpenGL;
			eyeTextures[i].OGL.Header.TextureSize = hmd->Resolution;
			eyeTextures[i].OGL.Header.RenderViewport.Size.w = hmd->Resolution.w/2;
			eyeTextures[i].OGL.Header.RenderViewport.Size.h = hmd->Resolution.h;
			eyeTextures[i].OGL.Header.RenderViewport.Pos.x = i * hmd->Resolution.w/2;
			eyeTextures[i].OGL.Header.RenderViewport.Pos.y = 0;
			eyeTextures[i].OGL.TexId = 0;
		}
		...
		//apiConfig.OGL.Header.RTSize = hmd->Resolution;
        ...
        for ( int eyeNum = 0; eyeNum < 2; eyeNum++ ) {
            ...
            eyeFovIn[eyeNum] = fullview ? hmd->MaxEyeFov[eyeNum] : hmd->DefaultEyeFov[eyeNum];
        }
        ovrEyeRenderDesc eyeRenderDescOut[2];
        
        ovrHmd_ConfigureRendering(hmd, (const ovrRenderAPIConfig*)&apiConfig, distortionCaps, eyeFovIn, eyeRenderDescOut);

        // TODO: probably all this is unnecessary -- but maybe it's useful for someone
        for ( int eyeNum = 0; eyeNum < 2; eyeNum++ ) {
	...
	void jit_gl_texture(t_symbol * texsym) {
		void * texob = jit_object_findregistered(texsym);
		if (texob) {
			texture = texsym;
		}
	}
	...
	void bang_sdk() {
		...
		ovrPosef headPose[2];
		void * texob;
		ovrFrameTiming timing = ovrHmd_BeginFrame(hmd, frameIndex++);
		
		texob = jit_object_findregistered(texture);
		if (!texob) { object_warn(&ob, "missing texture"); return; }
		
		jit_attr_getlong_array(texob, _jit_sym_dim, 2, dim);
		...
		for (int eyeIndex = 0; eyeIndex < ovrEye_Count; eyeIndex++) {
			...
			jit_attr_getlong(texob, ps_glid);
			eyeTextures[eye].OGL.Header.TextureSize.w = dim[0];
			eyeTextures[eye].OGL.Header.TextureSize.h = dim[1];
			eyeTextures[eye].OGL.TexId = jit_attr_getlong(texob, ps_glid);
			
			ovrQuatf orient = headPose[eye].Orientation;
			...
			//object_post(&ob, "texture %u %u %u", eyeTextures[eye].OGL.TexId, dim[0], dim[1]);
		}
		
		ovrHmd_EndFrame(hmd, headPose, (const ovrTexture *)eyeTextures);
	}
...
void oculus_jit_gl_texture(t_oculus * x, t_symbol *name) {
	x->jit_gl_texture(name);
}
...
int C74_EXPORT main(void) {	
    ...
    // TODO: when should ovr_ShutDown() be called? When the external is unloaded...
    ...
    class_addmethod(maxclass, (method)oculus_jit_gl_texture, "jit_gl_texture", A_SYM, 0);
    ...
    CLASS_ATTR_LONG(maxclass, "sdk", 0, t_oculus, sdk);
    CLASS_ATTR_STYLE_LABEL(maxclass, "sdk", 0, "onoff", "use sdk rendering");
    CLASS_ATTR_LONG(maxclass, "multisample", 0, t_oculus, multisample);
    CLASS_ATTR_STYLE_LABEL(maxclass, "multisample", 0, "onoff", "use multisampling in sdk rendering");
